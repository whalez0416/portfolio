...
| `node test/mobile.js` | **폰 화면에서 성립하는가** (레이아웃·터치 경로) | Chrome + puppeteer-core + `:8000` 서버 |
...
- **`mobile.js`** — 작은 화면에서 **나침반이 연도 표기를 덮던 것.**

`mobile.js` 참고: 헤드리스 모바일 에뮬레이션의 네이티브 터치 합성(touchscreen.tap)은
히트테스트 좌표가 스크롤에 따라 밀리는 아티팩트가 있어(사이트 버그 아님) 쓰지 않고,
패드가 실제로 듣는 PointerEvent 를 디스패치합니다. 브라우저의 탭→click 합성은 표준
동작이라 실기로 봅니다 — 같은 공유기에서 폰으로 `http://<PC IP>:8000` 을 열면 됩니다.
